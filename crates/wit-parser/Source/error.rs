...
pub enum Error {
	...
	/// Disallow several characters which are deprecated or discouraged in
	/// Unicode.
	///
	/// U+149 deprecated; see Unicode 13.0.0, sec. 7.1 Latin, Compatibility
	/// Digraphs. U+673 deprecated; see Unicode 13.0.0, sec. 9.2 Arabic,
	/// Additional Vowel Marks. U+F77 and U+F79 deprecated; see Unicode 13.0.0,
	/// sec. 13.4 Tibetan, Vowels. U+17A3 and U+17A4 deprecated, and U+17B4 and
	/// U+17B5 discouraged; see Unicode 13.0.0, sec. 16.4 Khmer, Characters
	...
	/// Types can't recursively refer to themselves as that would make then
	/// possibly infinitely-sized. In Rust the compiler would force you to use
	/// heap indirection, however such a thing doesn't exist in out type system.
	///
	/// This wouldn't be a problem with the current JSON format, but a custom
	/// binary one would have this limitation so for future proofing we deny
	/// recursive types.
...
impl From<Vec<Error>> for Error {
	fn from(errors:Vec<Error>) -> Self { Self::Multi { errors } }
}
...
impl Error {
	...
	pub fn empty_type(loc:impl Into<Span>, ty:Token) -> Self {
		Self::EmptyType { location:loc.into(), ty }
	}
	...
	pub fn already_defined(loc:impl Into<Span>, prev:impl Into<Span>) -> Self {
		Self::AlreadyDefined { previous:prev.into(), location:loc.into() }
	}
